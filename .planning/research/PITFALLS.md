...
**What goes wrong:** If a worktree setup sequence fails mid-way (e.g., `git worktree add` succeeds but a subsequent step like git-crypt unlock or state file write throws an error), the worktree directory and its `.git/worktrees/` metadata entry are left permanently registered. Subsequent runs with the same issue number generate a _new_ random-ish name instead of reusing the old path. You accumulate phantom worktrees that block branch operations.
...
- Write a `WORKTREE_READY` flag into the state file only as the _last_ step of setup. On resume/recovery, absence of this flag signals incomplete setup to clean up and retry.
...
- The cleanup function must always go through `git worktree remove` first, then `git worktree prune`, _then_ attempt branch deletion.
...
- Always call `git push -u origin "$branch"` explicitly _before_ `gh pr create`, with `origin` hardcoded as the remote name.
...
- Never run `git fetch` while Claude is actively running in the worktree. Do all fetching _before_ launching Claude.
...
- If `origin/main` is ahead, automatically rebase the feature branch onto it _before_ showing the diff review. Abort with a clear message if the rebase has conflicts.
