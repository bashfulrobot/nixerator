# Text Polish Voice Integration - Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split text-polish into two keybindings: Super-Shift-R (voiced, Dustin's writing style) and Super-Ctrl-R (generic, current behaviour).

**Architecture:** Two independent bash scripts sharing the same shell structure but different prompts. One Nix module producing two Hyprland conf.d drop-in files and two `replaceVars` derivations.

**Tech Stack:** Bash, Claude CLI (`claude -p`), Nix (replaceVars, Home Manager, Hyprland conf.d), wl-clipboard, libnotify.

---

## File map

- Create: `modules/apps/cli/text-polish/scripts/text-polish-voice.sh` -- voiced polish script
- Modify: `modules/apps/cli/text-polish/default.nix:9-29` -- add second script derivation and keybinding
- Unchanged: `modules/apps/cli/text-polish/scripts/text-polish.sh` -- generic polish (no edits)

---

### Task 1: Create the voiced polish script

**Files:**
- Create: `modules/apps/cli/text-polish/scripts/text-polish-voice.sh`

- [ ] **Step 1: Create `text-polish-voice.sh`**

This is the full script. Shell logic is duplicated from `text-polish.sh`; only the prompt differs.
...
Run: `chmod +x modules/apps/cli/text-polish/scripts/text-polish-voice.sh`

- [ ] **Step 3: Commit**

```bash
git add modules/apps/cli/text-polish/scripts/text-polish-voice.sh
git commit -m "feat(text-polish): add voiced polish script with writing-style integration"
```

---

### Task 2: Update the Nix module for dual keybindings

**Files:**
- Modify: `modules/apps/cli/text-polish/default.nix`

- [ ] **Step 1: Update `default.nix` to add second script and split keybindings**
...
- [ ] **Step 2: Verify the Nix module evaluates correctly**

Run: `just quiet-rebuild`

Expected: Build succeeds. If it fails, spawn a Nix subagent to read `/tmp/nixerator-rebuild.log` and diagnose.

- [ ] **Step 3: Commit**

```bash
git add modules/apps/cli/text-polish/default.nix
git commit -m "feat(text-polish): split keybindings — voiced (Super-Shift-R) and generic (Super-Ctrl-R)"
```
...
- [ ] **Step 1: Test voiced polish (Super-Shift-R)**

1. Open a text editor or Slack
2. Type: "Hey Dimitri, I wanted to follow up on the issue we discussed. I think that basically the problem is that the system is not behaving in the way that we expected it to. Can you take a look at it when you get a chance? Thanks"
3. Select the text
4. Press Super-Shift-R
5. Verify: output uses Canadian English, ends with "Cheers,", concise, sounds like Dustin

- [ ] **Step 2: Test generic polish (Super-Ctrl-R)**

1. Select the same text
2. Press Super-Ctrl-R
3. Verify: output is concise, preserves original tone (no "Cheers,", no voice injection)

- [ ] **Step 3: Test edge cases**

1. Select nothing, press Super-Shift-R -- verify error notification
2. Select nothing, press Super-Ctrl-R -- verify error notification
3. Select a very short Slack-style message ("sounds good"), press Super-Shift-R -- verify casual output
