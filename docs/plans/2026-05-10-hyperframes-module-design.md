...
| `modules/apps/cli/claude-code/default.nix` | (1) Add `hasHyperframes = lib.elem "hyperframes@hyperframes" cfg.plugins;` in the `let` block. (2) Conditionally extend `environment.systemPackages` with `[ pkgs.ffmpeg-full pkgs.chromium ]`. (3) Conditionally extend `environment.variables` with `{ PUPPETEER_EXECUTABLE_PATH = "${pkgs.chromium}/bin/chromium"; PUPPETEER_SKIP_DOWNLOAD = "1"; }`. (4) Conditionally extend `home-manager.users.${globals.user.name}.home.packages` with `[ pkgs.nodejs_22 ]`. |
...
- **Environment variables (system-wide)**: `PUPPETEER_EXECUTABLE_PATH` pointing at `${pkgs.chromium}/bin/chromium`, `PUPPETEER_SKIP_DOWNLOAD=1` to suppress puppeteer's installer trying to fetch its own binary into a writable path.
