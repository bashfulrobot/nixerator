...
- **Environment variables**: `PUPPETEER_EXECUTABLE_PATH = "${pkgs.chromium}/bin/chromium"` and `PUPPETEER_SKIP_DOWNLOAD = "1"`, set in both `environment.variables` and `home.sessionVariables` so they're visible to system-level shells *and* the user's interactive sessions where `npx hyperframes` actually runs.
