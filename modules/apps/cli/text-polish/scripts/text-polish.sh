...
main() {
  ...
  # 4. Build the prompt. Static concision rules first, then the output contract.
  # The rules live in @rules_file@ (Nix store path substituted at build), a single
  # source of truth shared with the claude-code `text-polish` skill so the two
  # cannot drift. Editing the rules file moves both the keybind and the skill.
  local PROMPT
