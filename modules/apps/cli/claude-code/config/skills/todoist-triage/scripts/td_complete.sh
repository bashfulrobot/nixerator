...
ref="${1:-}"; [ -n "$ref" ] || { echo "usage: td_complete.sh <ref> --reason \"<why>\" [--forever] [--dry-run]" >&2; exit 2; }
shift
reason=""; forever=0; dry_run=0
while [ $# -gt 0 ]; do
  case "$1" in
...
[ -n "$reason" ] || { echo "--reason is required (record why it's done)" >&2; exit 2; }

wl=("$ref" --verb complete --entry "Completed. ${reason}")
[ "$dry_run" -eq 1 ] && wl+=(--dry-run)
bash "$SCRIPT_DIR/td_worklog.sh" "${wl[@]}"

cflags=(); [ "$forever" -eq 1 ] && cflags+=(--forever); [ "$dry_run" -eq 1 ] && cflags+=(--dry-run)
