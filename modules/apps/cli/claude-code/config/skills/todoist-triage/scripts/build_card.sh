...
render_card() { # position column auto ; json on stdin
  ...
  local due_str
  if [ -n "$due" ]; then
    local od; od=$(days_since "$due")
    if   [ -n "$od" ] && [ "$od" -gt 0 ]; then due_str="due ${due} (${od}d overdue)"
    elif [ -n "$od" ] && [ "$od" -eq 0 ]; then due_str="due today"
  ...
  local blob; blob="$title"$'\n'"$(jq -r '.comments[].content' <<<"$json")"
  local crumbs; crumbs=$(printf '%s' "$blob" | extract_breadcrumbs \
    | awk -F'\t' '{printf "%s %s · ", $1, $2}' | sed 's/ · $//')

  local last2; last2=$(jq -r '
    .comments | (sort_by(.posted_at) | reverse) | .[0:2][]
    | "\((.posted_at // "?")[0:10])  \((.content | gsub("\n";" "))[0:70])"' <<<"$json")

  local newest; newest=$(jq -r '.comments | (sort_by(.posted_at)|reverse)|.[0].posted_at // ""' <<<"$json")
  local lastmv=""; local mv; mv=$(days_since "${newest:0:10}")
  [ -n "$mv" ] && lastmv="last movement ${newest:0:10} (${mv}d)"

  local hedges; hedges=$(jq -r '.comments[].content' <<<"$json" | harvest_hedges | head -4 | sed 's/^/    · /')
