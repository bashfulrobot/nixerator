...
ref="${1:-}"; [ -n "$ref" ] || { echo "usage: td_drop.sh <ref> --reason \"<why>\" [--dry-run]" >&2; exit 2; }
shift
reason=""; dry_run=0
while [ $# -gt 0 ]; do
  case "$1" in
