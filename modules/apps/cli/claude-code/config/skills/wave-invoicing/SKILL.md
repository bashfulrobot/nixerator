...
description: >-
  ...
  email -- Wave never emails the customer either, even when later asked to
  approve/mark-sent the invoice after the user has emailed it themselves.
when_to_use: >-
...
`wave-download-pdf.sh`, `wave-invoice-approve.sh`, `wave-invoice-mark-sent.sh`)
need this setup before they work:
...
10. Build line items JSON: consulting items use `products.consulting`
    (`quantity`=hours, `unitPrice`=rate); pass-through items use
    `products.passthrough` (`quantity`=1, `unitPrice`=amount — the field is
    `unitPrice`, not `price`). Then in one shell:
    `source scripts/lib.sh config.json; build_invoice_payload <BIZ> <CUST> <NUMBER> <INVOICE_DATE> <DUE_DATE> <ITEMS>` > "$(mktemp)"
    (capture the mktemp path as `PAYLOAD_FILE`; ask for invoice date / due
    terms; default due = invoice date + 30d).
11. Create the DRAFT: `scripts/wave-create-invoice.sh "$PAYLOAD_FILE"` → capture
    `id`, `pdfUrl`, `invoiceNumber` into shell variables, then `rm -f
    "$PAYLOAD_FILE"`. **Working-folder discipline:** never write the Wave API
    request payload or the raw mutation response into the working folder —
    they're scratch, not deliverables. The working folder must end up holding
    only the per-vendor evidence subfolders, the invoice PDF, the zip, and
    `email.md` (matches the historical Insync layout, e.g. `2026/08/`).
...
### Stage 2b — Mark it sent in Wave (offer, gated)
18. Also offer, separately, to update the invoice's status in Wave now that
    it's actually been emailed:
    a. `scripts/wave-invoice-approve.sh <invoiceId>` — DRAFT → Approved. No
       customer contact of any kind.
    b. `scripts/wave-invoice-mark-sent.sh <invoiceId>` — records it as sent
       (`sendMethod: MARKED_SENT`, since the user sent it by hand via Gmail,
       not through Wave). Also no customer contact.
    Run both only on explicit per-invoice confirmation, same bar as an Insync
    file-in. **Never call `invoiceSend`** (no script wraps it) — that mutation
    has Wave itself email the customer, which would double-send on top of the
    user's own cover email and bypass their voice entirely.
