...
description: >-
  Build a Kong customer's renewal-projection or consumption-analysis slide deck
  from their usage data. Pulls the contracted entitlement from Salesforce,
  decodes the SKU units (the easy thing to get wrong), reconciles measured
  actuals against customer projections and licensed entitlement, then renders a
  Kong-branded PPTX with data-provenance speaker notes.
when_to_use: >-
  Use whenever the job is to build, model, or visualize a renewal projection, a
  consumption or growth deck, an API-usage projection, or renewal-readiness
  slides for a Kong account: "turn these usage numbers into a renewal deck",
  "build the renewal slides", "model the renewal at current service count",
  "project their API growth into the renewal", "consumption growth deck", or
  "renewal analysis, licensed vs used", including pasting raw customer usage
  stats and asking for a renewal visual. Prefer this over kong-technical-csm and
  kong-pptx whenever the specific job is a renewal or consumption deck built
  from usage data, and over csp-draft unless the user asks for a success plan.
  Do NOT trigger for renewal strategy questions with no deck
  (kong-technical-csm), success-plan decks (csp-draft), meeting prep
  (meeting-prep), CSAT updates (csat), plain Salesforce queries (sfdc), or
  generic non-renewal pitch decks (kong-pptx).
effort: high
