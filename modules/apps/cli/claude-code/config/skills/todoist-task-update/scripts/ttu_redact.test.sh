...
clean() { # name text  -> expect exit 0
  if printf '%s' "$2" | bash "$here/ttu_redact.sh" >/dev/null 2>&1; then
    echo "PASS: $1"
    pass=$((pass + 1))
  else
    echo "FAIL: $1 (should be clean)"
    fail=$((fail + 1))
  fi
}
dirty() { # name text  -> expect exit 3 AND no value leak on stderr
  err=$(printf '%s' "$2" | bash "$here/ttu_redact.sh" 2>&1 >/dev/null)
  rc=$?
  if [ "$rc" -eq 3 ] && ! printf '%s' "$err" | grep -qF "$2"; then
    echo "PASS: $1"
    pass=$((pass + 1))
  else
    echo "FAIL: $1 (rc=$rc, or value leaked)"
    fail=$((fail + 1))
  fi
}
