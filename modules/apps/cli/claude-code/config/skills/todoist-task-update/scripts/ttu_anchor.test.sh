...
out=$(printf '%s' '{"error":{"code":"INTERNAL_ERROR","message":"HTTP 502: Bad Gateway"}}' | bash "$here/ttu_anchor.sh" 2>/dev/null); rc=$?
eq "error payload exit 4"   "4"  "$rc"
eq "error payload no anchor" ""  "$out"

# Non-object / garbage input → exit 4, no anchor.
out=$(printf '%s' 'not json at all' | bash "$here/ttu_anchor.sh" 2>/dev/null); rc=$?
