...
# token is a recovery subcommand. Catches quoted forms (git "stash") and
# option-prefixed forms (git -C /path stash).
if grep -qP '(^|\s|;|&&|\|)git(\s+-\S+(\s+[^-]\S*)?)*\s+["'\'']?stash\b(?!\s+(pop|apply|list|show|drop)\b)' <<<"$cmd"; then
  jq -nc '{
    hookSpecificOutput: {
